...
class TestProcessPendingSessions:
    ...
    async def test_processes_each_session(self, mock_process) -> None:
        ...
        await process_pending_sessions(session_mgr)
    ...
    async def test_returns_summary(self, mock_process) -> None:
        mock_process.side_effect = [
            ...
            {"session_id": "s2", "status": "done", "write_mode": "auto", "memories_written": 1, "entities_written": 0, "errors": 0},
        ...
        result = await process_pending_sessions(session_mgr)

        assert result["processed"] == 3
        assert result["auto_written"] == 2
    ...
    @patch("core.epilogue.process_session")
    async def test_returns_exception_count(self, mock_process) -> None:
        mock_process.side_effect = [
            {
                "session_id": "s1", "status": "done", "write_mode": "auto",
                "memories_written": 2, "entities_written": 1, "errors": 0,
                "exceptions": [{"trigger": "high_novel_entities", "detail": "11 novel entities"}],
            },
            {
                "session_id": "s2", "status": "done", "write_mode": "auto",
                "memories_written": 1, "entities_written": 0, "errors": 0,
            },
        ]
        session_mgr = AsyncMock()
        session_mgr.get_sessions_pending_epilogue = AsyncMock(return_value=[
