...
**For Person entities:** use `search_person`. Pass whatever name fragments are known as named params — `first_name`, `last_name`, `title`, `relationship`. All use case-insensitive substring matching. Pass only what you know; omit the rest.

Examples:
- Know their first name → `search_person(first_name="Manny")`
- Know their title → `search_person(title="Coach")`
- Know their relationship to Daniel → `search_person(relationship="wife")`
- Know first + last → `search_person(first_name="Wil", last_name="Vark")`
- Natural language like "my kids" → try `search_person(relationship="child")`, then `relationship="children"` if no results

Results are returned as a `matches` array.

**For all other entity types** (Project, System, Concept, etc.): use `graph_query` with the entity name or fragment.
