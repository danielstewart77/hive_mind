...
class FishTokenizer:
    def __init__(
        self, model_path: str, special_tokens=ALL_SPECIAL_TOKENS
    ) -> None:
        mergeable_ranks = self.load_tiktoken_bpe(str(model_path))

        if isinstance(special_tokens, dict):
            # from_pretrained passes pre-built dict from special_tokens.json
            self.all_special_tokens_with_ids = special_tokens
        else:
            # list of token strings — build the dict
        ...
        end_idx = 0
        for token, tid in self.all_special_tokens_with_ids.items():
            if token.startswith("<|semantic:"):
                idx = int(re.match(r"<\|semantic:(\d+)\|>", token).group(1))
                self.semantic_id_to_token_id[idx] = tid
                if idx > end_idx:
                    end_idx = idx

        if self.semantic_id_to_token_id:
    ...
    def pad_token_id(self):
        return self.all_special_tokens_with_ids.get(PAD_TOKEN, 0)

    @property
    def eos_token_id(self):
        return self.all_special_tokens_with_ids.get(EOS_TOKEN, 0)
    ...
    def encode(self, s: str, add_special_tokens=False, allowed_special=True, **kwargs) -> list[int]:
